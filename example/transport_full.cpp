...
int main(int argc, char ** argv) {
  ...
  if (argc != 5) {
    std::cout <<
      "Usage: " << argv[0] << " file_A file_B percent_B output_file"
      << std::endl;
    return 1;
  }

  // Get the percents
  double fraction = std::atof(argv[3])/100.;
  ...
  for (size_t c = 0; c < num_channels; c++) {
    ...
    for (size_t w = 0; w < num_windows; w++) {
      ...
      double interpolation_factor = fraction;
  ...
  // Write the file
  std::cout << "Writing to file " << argv[4] << std::endl;
  audiorw::write(audio_interpolated, argv[4], sample_rate);
}
